...
void* send_messages(void* arg) //(это значит ф-я используется в потоке)
{
    ...
    while (1) 
    {
        ...
        *space = 0;
        ...
        pthread_mutex_lock(&users_mutex);
        for (int i = 0; i < usercount; i++) {
            if (strcmp(users[i].nick, ip) == 0 && users[i].online) {
                strncpy(target_ip, users[i].ip, 15);
                break;
            }
        }
        pthread_mutex_unlock(&users_mutex);
...
int main() 
{
    ...
    while (1) 
    {
        ...
        if (msg.type == MessageType_Hello) {
            if (strcmp(msg.hello.nickname, mynick) == 0) continue;
            ...
            if (strcmp(msg.privmsg.to, mynick) == 0) {
                ...
                printf("\n%s[личное] %s%s%s%s: %s%s\n", 
                       BOLD,
                       get_color_for_nick(msg.privmsg.from),
                       msg.privmsg.from, 
                       RESET,
                       BOLD,
                       msg.privmsg.text,
