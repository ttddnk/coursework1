...
struct User { //один пользователь
    char ip[50];
    char nikname[100];
};

struct User users[100]; //масив для всех поль
int userscol = 0;

char* getnikname(char* ip) { //поиск ника по айпи
...
void adduser(char* ip, char* nikname) { //новый пользователь
    strcpy(users[userscol].ip, ip);
    strcpy(users[userscol].nikname, nikname);
    userscol++;
}

char nowip[50]; // общие переменные 
char nownikname[100];

void opruser() { //опр поль
    char ip[50];
    char nikname[100];
    char* sushnikname; //возр ник
    
    printf("Введите ваш ip");
    fflush(stdout);
    scanf("%s", ip);
    
    sushnikname = getnikname(ip); //запр
    
    if (sushnikname != NULL) { //проверяем есть ли + вводим если нет
        printf("Ваш никнейм %s\n", sushnikname);
        strcpy(nownikname, sushnikname); //копируемм чтоы использовать потом
        strcpy(nowip, ip);
    } else {
        ...
        adduser(ip, nikname); //добавляем если нет
        
        strcpy(nownikname, nikname);//копируемм чтоы использовать потом
        strcpy(nowip, ip);
    }
}
...
void* send_messages(void* arg) //(это значит ф-я используется в потоке)
{
    ...
    dest.sin_port = htons(8888); //ставим порт
    ...
    while (1) 
    {
        ...
        char nikmsg[356]; // сообщение с ником
        snprintf(nikmsg, sizeof(nikmsg), "[%s]: %s", nownikname, msg);

        sendto(sock, nikmsg, strlen(nikmsg), 0, (struct sockaddr*)&dest, sizeof(dest)); //отправка сообщения
        printf("Sended\n");
        fflush(stdout);
    }
...
int main() 
{
    ...
    opruser(); //опр при входе


    addr.sin_family = AF_INET; //адрес будет IPv4
    addr.sin_port = htons(8888); //порт 8882(сетевой порядок байт)
